...
import { Upload, FileText, Sparkles, Trash2, Download } from "lucide-react";
...
function DocumentsPage() {
  ...
  async function handleAssign(doc: Doc, clientId: string) {
    ...
    <div>
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="mt-2 text-foreground/70">Drop in receipts, contracts, anything.</p>
        </div>
      </div>
      ...
      >
        ...
        <p className="mt-1 text-sm text-foreground/60">or click to browse · PDF, images, docs · max 25MB</p>
      </div>
      ...
      <div className="mt-8">
        {loading ? (
          ...
          <div className="space-y-2">
            {docs.map((d) => (
              <div key={d.id} className="rounded-2xl bg-card p-4 ring-1 ring-warm flex items-center gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-foreground/60 mt-0.5">
                    ...
                    {d.status === "processed" && <span className="ml-2 inline-flex items-center gap-1 text-rose-deep"><Sparkles className="h-3 w-3" />AI ready</span>}
                  </p>
                </div>
                <select
                  value={d.client_id ?? ""}
                ...
                <button onClick={() => handleDownload(d)} className="p-2 rounded-full hover:bg-background text-foreground/60 hover:text-foreground" title="Open">
                  <Download className="h-4 w-4" />
                </button>
                <button onClick={() => handleDelete(d)} className="p-2 rounded-full hover:bg-destructive/10 text-foreground/60 hover:text-destructive" title="Delete">
